...
function getEntryFiles() {
    ...
    if (darkMode) {
        entries['plugins/global/plugins.dark.bundle'] = ['./webpack/plugins/plugins.scss'];
        entries['css/style.dark.bundle'] = ['./' + path.relative('./', srcPath) + '/sass/style.dark.scss', './' + path.relative('./', srcPath) + '/sass/plugins.dark.scss'];

        if (isPresets) {
            if (fs.existsSync(srcPath + '/sass/presets')) {
                presets.forEach(preset => {
                    entries['plugins/global/plugins.' + presets + '.dark.bundle'] = ['./webpack/plugins/plugins.scss'];
                    entries['css/style.' + presets + '.dark.bundle'] = ['./' + path.relative('./', srcPath) + '/sass/presets/' + preset + '/style.style.dark.scss', './' + path.relative('./', srcPath) + '/sass/presets/' + preset + '/plugins.dark.scss'];
                });
            }
        }
    }
...
function getParameters() {
    var possibleArgs = [
        ...
        'js', 'css', 'scss',
        'alldemos', 'rtl', 'prod', 'production', 'localhost', 'dark_mode', 'presets',
    ];
    for (var i = 0; i <= 25; i++) {
